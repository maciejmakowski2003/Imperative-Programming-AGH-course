...
void ptab_sort(char *ptab[], size_t n) {
    qsort(ptab,n,sizeof(const char*),compar);
}
...
void print_t2D_ind(const char (*ptr)[STRLEN_MAX], const size_t *pindices, size_t n) {
    for(int i=0;i<n;i++){
        for(int j=0;ptr[pindices[i]][j]!='\0';j++){
...
void read_mat51(double A[][SIZE], size_t m, size_t n) {
    for(size_t i=0; i<m; ++i) {
        for(size_t j=0; j<n; ++j)  scanf("%lf",&A[i][j]);
    }
}

void print_mat51(const double A[][SIZE], size_t m, size_t n) {
    for(size_t i=0; i<m; ++i) {
